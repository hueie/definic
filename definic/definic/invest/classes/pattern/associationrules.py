...
class Associationrules:
    #Finding Association Rules ( If Someone bought A product, What products might he buy with that. {A} => {B} ? {C} ? ... )
...
if __name__ == "__main__":
    ...
    associationrules.train(x_train, minimum_support_threshold, minimum_confidence_threshold)
    print("\n")
    for row in associationrules.output:
        print(row)
        #print(row['A'])
        
    #associationrules.generating_subset(set({}), set({"a" , "b" , "c" ,"d" }))
